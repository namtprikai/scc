<template>
	<div>
		<TabHeader>
			<b-button size="sm" v-b-toggle.collapse-3 class="mr-2"
				>画像新規追加</b-button
			>
		...
		<div class="fileUpload__upload-list">
			...
			>
				...
				<div class="fileUpload__body">
					...
					<div
						class="
							fileUpload__upload-text fileUpload__upload-text--date
							mt-1
							text-right
						"
					>
				...
				<div class="fileUpload__footer card-footer">
					<b-button inline size="sm" class @click="emitUrl(file.Key)">挿入</b-button>
					<b-button inline size="sm" v-on:click="deleate(file.Key)" class="ml-1"
						>削除</b-button
					>
				</div>
			</div>
...
export default class MediaListComp extends FileuploadCompParent {
	emitUrl(fileName: string) {
		this.$bvToast.toast("挿入されました", {
			// title: `挿入されました`,
		...
		eventHub.$emit(
			"selectImage",
			`https://file.ai-x-supporter.com/${CLIENT_ID}/${fileName}`
		);
	}
}
</script>
