	...
	<div>
		<div class="text-center">
			...
			<b-form-file
				...
				multiple
			></b-form-file>
			<span class="fileUpload__caption text-discription"
				>※ファイルサイズの上限は5MBです</span
			>
			<b-alert show variant="warning" v-if="IsInValidFilename"
				>ファイル名に使用してはいけない文字が含まれています。</b-alert
			>
			...
			<div class="text-center mb-3">
				<b-button
					...
					v-bind:disabled="uploadFileList == null || IsInValidFilename"
					>アップロード</b-button
				>
			</div>
		</div>
...
export default class FileuploadComp extends FileuploadCompParent {
	get IsInValidFilename() {
		return (
			this.uploadFileList?.find((f: File) => !this.filenameValidate(f.name)) ||
			this.fileNameList.find((f: string) => !this.filenameValidate(f)) ||
			false
		);
	}
