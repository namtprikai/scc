<template>
	<div>
		<b-list-group>
			<b-list-group-item
				v-for="anser in answers"
				:key="anser.id"
				@click="setAnswer(anser)"
				class="d-block"
			>
				<p>
					<span v-for="(conditionObj, key) in anser.anserConditionMap" :key="key">
						<b-badge
							v-for="(condition, index) in conditionObj.conditions"
							:key="index"
							>{{ condition.label }}</b-badge
						>
					</span>
				</p>
