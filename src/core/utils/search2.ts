	...
	for (let i = 0; i < scenarioList.length; i++) {
		...
		search(
			...
			(ob: { flow: ScenarioFlow; depth: number; y: number; parentFlow?: any }) => {
				...
				while (row.length - 1 <= ob.depth) {
					row.push("");
				}
				row[ob.depth] = ob.flow.step;
...
export const horizontaltojson = (bot_csv: any) => {
	...
	for (let h = 0; h < rootKeyList.length; h++) {
		...
		start([rootKeyList[h]], (o_: any, i: any) => {
			...
			if (is_null(bot_csv[o_.y][o_.x + 1])) {
				nextNumber = 0;
			} else if (o_.x === 0) {
				nextNumber = 2;
			...
			const log_itemsMATCH = conditionTEXT.match(/<log-(.+?):(.+?)>/g);
