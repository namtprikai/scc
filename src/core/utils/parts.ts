...
export function AutoScenarioLogTag({ TalkScript, ScenarioList }: { TalkScript: Array<TalkScript>; ScenarioList: Array<any> }) {
	const scenarioList = JSON.parse(
		JSON.stringify(
	...
	for (const item of TalkScript) {
		if (item.type === 'leaf' && item.items && item.items.scenario_id) {
			const ancestorList = [item];
			const scenarioId: any = item.items.scenario_id;
	...
	for (const scenario of scenarioList) {
		const rootFlow = scenario.flow[0];
		const { scenarioId } = scenario;
		search(
			rootFlow,
			(step: any, parents: Array<any>) => {
				// for (const _step of parents) {
				if (scenarioId in linkScriptMap) {
					...
					if (
						parents
							...
							.filter((o, i) => i !== 0).length > 0
					) {
						logName = `${scenarioId}_${parents
							.map(o => o.position)
							.filter(o => o !== undefined)
							.filter((o, i) => i !== 0)
							.join('_')}`;
					...
					console.log(stepKey);
					const currentStep = scenario.step[stepKey];
					delete scenario.step[stepKey].items;
					// scenario.steps[stepKey].action.success.items={'log_faq':[logName]};
					// console.log(scenario.steps[stepKey].action.success);
					scenario.step[stepKey].items = {
						log_scenario: [logName],
						log_faq: [scenarioId],
						log_faq_parent_category: [scriptList[0].text],
						log_faq_child_category: [scriptList[1].text],
						log_faq_title: [scriptList[scriptList.length - 1].text],
					};
				}
				// }
			},
			[{ step: rootFlow, position: count }],
		);
		count++;
	}
	function search(step: any, fn: Function, parents: any = []) {
		fn(step, parents);
		let count = 0;
		for (const nextStep of step.next) {
			const nParents: Array<any> = [].concat(parents);
			nParents.push({ step: nextStep, position: count });
			search(nextStep, fn, nParents);
			count++;
		}
	}
	return scenarioList;
}
export function AutoBotLogTag({ TalkScript, BotConfig }: { TalkScript: Array<TalkScript>; BotConfig: any }) {
	const { scenario } = JSON.parse(JSON.stringify(BotConfig));
	console.log(scenario);
	const rootStep = scenario.flow.root;
	const linkScriptMap: any = {};
	...
	let count = 0;
	for (const rStep of rootStep.next) {
		const scenarioId = rStep.condition.value;
		search(
			rStep,
			(step: any, parents: Array<any>) => {
				if (step.step == 'C_1') {
				...
				if (scenarioId in linkScriptMap) {
					const scriptList = linkScriptMap[scenarioId];
					console.log(stepKey);
					const currentStep = scenario.steps[stepKey];
					delete scenario.steps[stepKey].action.success.items;
					// scenario.steps[stepKey].action.success.items={'log_faq':[logName]};
					// console.log(scenario.steps[stepKey].action.success);
					scenario.steps[stepKey].action.success.items = {
						log_scenario: [logName],
						log_faq: [scenarioId],
						log_faq_parent_category: [scriptList[0].text],
						log_faq_child_category: [scriptList[1].text],
					...
					const labelValue = String(step.label).replace(/\d+?\./, '');
					if (labelValue) {
						for (const linkScript of linkScriptTitleMap) {
							if (linkScript.key == labelValue) {
								const scriptList = linkScript.value;
								scenario.steps[stepKey].action.success.items = {
									log_scenario: [logName],
									log_faq: [scenarioId],
									log_faq_parent_category: [scriptList[0].text],
...
export function AutoScriptLogTag(TalkScript: Array<TalkScript>) {
	...
	for (const item of talkScript) {
		if (item.parent === '#') {
			...
			let type = item.type;
			const addItems: any = {};
			for (let i = 0; i < ancestorList.length && i < logMapper.length; i++) {
				addItems[logMapper[i]] = [ancestorList[i].text];
			}
			...
			while (parent.length > 0) {
				flg = false;
				let depthCount = 0;
				const _parent = parent.shift();
				for (let i = 0; i < talkScript.length; i++) {
					const jItem = talkScript[i];
					if (jItem.parent === _parent) {
						if (depthCount == 0) {
							ancestorList.push(jItem);
						} else {
							ancestorList[ancestorList.length - 1] = jItem;
						}
						const addItems: any = {};
						for (let i = 0; i < ancestorList.length && i < 2; i++) {
							addItems[logMapper[i]] = [ancestorList[i].text];
						}
						if ('scenario_id' in jItem.items) {
							jItem.items.log_scenario = [jItem.items.scenario_id];
							jItem.items.log_faq = [jItem.items.scenario_id];
						}
						jItem.items = Object.assign(
							{},
							jItem.items || {},
							{
								log_faq_title: [jItem.text],
							},
							addItems,
						);
						if (jItem.type == 'leaf') {
							console.log(jItem.items);
						}
						parent.push(jItem.id);
						type = jItem.type;
						flg = true;
						depthCount++;
					}
