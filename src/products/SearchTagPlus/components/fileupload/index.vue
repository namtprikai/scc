<template>
	<div>
		<div class="text-center">
			...
			<span class="fileUpload__caption text-discription">※ファイルサイズの上限は5MBです</span>
			<b-alert show variant="warning" v-if="IsInValidFilename">ファイル名に使用してはいけない文字が含まれています。</b-alert>
			<label id="output"></label>
			<div class="text-center mb-3">
				<b-button variant="primary" v-on:click="confirmOpened = true" v-bind:disabled="uploadFileList == null || IsInValidFilename">アップロード</b-button>
			</div>
		</div>
				...
				<div class="modal-wrapper">
					<div class="modal-container">
						...
						<div class="modal-footer">
							...
							<b-button variant="primary" @click="ok()" v-bind:disabled="uploadFileList == null || IsInValidFilename">OK</b-button>
						</div>
					</div>
				</div>
			</div>
...
export default class FileuploadComp extends FileuploadCompParent {
	get IsInValidFilename() {
		return this.uploadFileList?.find((f: File) => !this.filenameValidate(f.name)) || this.fileNameList.find((f: string) => !this.filenameValidate(f)) || false;
	}
	...
	public inputFile(file: any) {
		if (file) {
			...
			if (!this.filenameValidate(file.name)) {
	...
	public async upload() {
		if (this.uploadFileList.find((f: File) => !this.filenameValidate(f.name)) || this.fileNameList.find((f: string) => !this.filenameValidate(f))) {
			this.$bvToast.toast('ファイル名に使用してはいけない文字が含まれています。', {
		...
		this.$bvToast.toast('アップロードが成功しました。', {
			// title: `挿入されました`,
			toaster: 'b-toaster-top-center',
			solid: true,
			appendToast: true,
