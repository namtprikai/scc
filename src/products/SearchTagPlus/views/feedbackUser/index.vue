	...
	<div>
		<b-row>
			<b-col lg="12" class="pb-2">
				<el-table v-loading="loadFlg" :data="Messages" element-loading-text="Loading" border fit highlight-current-row @row-click="setCurrentMessage">
					...
					this.messages = res.data.filter((feedback: any) => {
						try {
							if (UserModule.name.indexOf('サイシード') != -1) {
								...
								JSON.parse(feedback.message.user.attribute || '{}').memo !== 'テストユーザー';
							}
						} catch (e) {
							console.log(e);
						}
