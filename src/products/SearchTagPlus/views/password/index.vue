<template>
	<div class>
		<TabHeader>
			<b-button variant="primary" size="sm" class="px-5" v-on:click="changepw()" :disabled="!isChangePassword">更新</b-button>
		</TabHeader>
		<div class="d-flex flex-column justify-content-center tab-body">
			<div class="container section">
				<b-form-group label-cols="4" label="現在のパスワード" label-for="password-input-1">
					<b-form-input id="password-input-1" size type="password" v-model="oldpw" placeholder="現在のパスワード"></b-form-input>
				</b-form-group>
				<b-form-group label-for="password-input-2" label-cols="4" :invalid-feedback="invalidFeedbackPassword" :valid-feedback="validFeedbackPassword" :state="statePassword">
					<template slot="label"
					>新しいパスワード
						...
						<b-popover :target="`pwinfo`" :placement="'left'" triggers="hover focus">
							<template slot:content>
								...
								<br />&lt;必須&gt; <br />・8文字以上 <br />&lt;以下のうち3つを満たすことが必須&gt; <br />・大文字を含む <br />・小文字を含む <br />・特殊文字を含む <br />・数字を含む
								<br />
							</template>
						</b-popover>
					</template>
					<b-form-input id="password-input-2" size type="password" v-model="newpw" placeholder="新しいパスワード"></b-form-input>
				</b-form-group>
				...
				>
					<b-form-input id="password-input-3" size type="password" v-model="newpw2" placeholder="新しいパスワード（確認用）" :state="matchPassword"></b-form-input>
				</b-form-group>
...
export default class Password extends Vue {
	...
	getValidMessageList(text: string) {
		...
		if (this.newpw !== '' && this.oldpw === this.newpw) {
			return ['今のパスワードと同じパスワードは設定できません'].concat(this.passwordValidator.getValidMessageList(text));
		}
