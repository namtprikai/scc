	...
	<div class="tab-body">
		...
		<!-- <b-list-group v-if="inv">
			<b-list-group-item class="text-center" v-for="(inverted,index) in inv" v-bind:key="index">
				...
				<el-slider :step="0.05" v-model="inverted.weight" :min="0" :max="10" disabled :show-input-controls='false'></el-slider>
		...
		>
			...
			<b-collapse :id="'collapse-d-' + index" visible>
				<el-slider
					:step="0.05"
	...
	private fetchInvertedIndex(time: number): Promise<any> {
		return new Promise((r: any) => {
			this.ajax
				.http({
					baseURL: `${packageUrl}`,
					url: `${PRODUCT_ID}/tag_package_test.json?${time}`,
					method: 'GET',
				})
				.then((res: any) => {
