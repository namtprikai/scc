<template>
	<b-container>
		<b-form>
			...
			<b-form-row class="mb-2">
				<b-col class="text-center">
					<b-button class="w-100" variant="outline-primary" v-bind:disable="loading" v-on:click="search">検索</b-button>
				</b-col>
			</b-form-row>
		</b-form>
		<div>
		...
		<el-table v-loading="loading" v-bind:data="messages" element-loading-text="検索中" border fit highlight-current-row v-on:row-click="setCurrentMessage">
...
export default class MessageSearch extends Vue {
	...
	public async download() {
		const dataList: any = this.messages.map(message => {
			...
			return [message.user.displayname || '-', message.created_date || '-', adminName || '-', message.text || '-'];
		});
